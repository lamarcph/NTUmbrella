...
public:
    ...
    float process(float input)
    {
        ...
        float leftCv = (leftEnv <= thrlin_ ? 1.f : powf(leftEnv * thrlinr_, expo_));
    ...
    void process(AudioBuffer &input, AudioBuffer &output)
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            // Compressor transfer function
            float leftCv = (leftEnv <= thrlin_ ? 1.f : powf(leftEnv / thrlin_, expo_));
            float rightCv = (rightEnv <= thrlin_ ? 1.f : powf(rightEnv / thrlin_, expo_));
