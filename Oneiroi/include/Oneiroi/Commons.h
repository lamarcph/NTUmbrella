...
inline float semi2Ratio(float semi){
    return Power(2.f, semi/12.f);
}
...
{
    bMin = logf(bMin < 0.0000001f ? 0.0000001f : bMin); // was fast log
    bMax = logf(bMax);

    return expf(Map(value, aMin, aMax, bMin, bMax));
}
...
{
    AudioBuffer* t = NTSampleBuffer::create(2, o.getSize());
...
private:
    ...
    void Expo()
    {
        ...
        {
            float x = d * i;
            lut_[i] = min_ + (max_ - min_) * (x == 1 ? 1 : powf(x, expoY_));
        }
    }
